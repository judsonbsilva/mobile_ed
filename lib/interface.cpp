...
void initStore(){
	...
	int length = LLSEcountApps(storeApps),
	...
	if( input == 0 ){	
		...
		input = LLSEgetTheIndex(input, storeApps);
		if( LLSEhasApp(storeApps.list[input].content, installedApps) > -1 ){
			cout << "   " << storeApps.list[input].content.name << " já instalado.\n";
			sleep(1000);
			initStore();
		} else {
			cout << "   " << storeApps.list[input].content.name << " instalado com sucesso.\n";
			LLSEinsertIn(storeApps.list[input].content, &installedApps);
				
			if( homeAppsLength < 10 )
				LLSEinsertIn(storeApps.list[input].content, &homeApps);
...
void initHome(){
	...
	if( input == 0 )
		...
		if( LLSEhasApp(homeApps.list[ input ].content, runningApps) > -1 ){
			cout << "   " << (homeApps.list[input].content.name) << " já está sendo executado.\n";
			sleep(1000);
			initHome();
		} else {
			cout << "   " << (homeApps.list[input].content.name) << " rodando.\n";
			LLSEinsertIn(homeApps.list[input].content, &runningApps);
			sleep(1000);
			initHome();
		}
	}
...
void initMyApps(){
	...
	int length = LLSEcountApps(installedApps);
	...
	if( input == 0 ){
		...
		input = LLSEgetTheIndex(input, installedApps);
		
		if( !uninstall && LLSEhasApp(installedApps.list[input].content, runningApps) > -1 ){
			cout << "   " << (installedApps.list[input].content.name) << " já está sendo executado.\n";
			sleep(1000);
			initMyApps();
		} else if( !uninstall ){
			cout << "   " << (installedApps.list[input].content.name) << " rodando.\n";
			LLSEinsertIn(installedApps.list[input].content, &runningApps);
			sleep(1000);
			initHome();
		}
		...
		if( uninstall ){
			...
			int appRunning = LLSEhasApp(installedApps.list[input].content, runningApps),
			...
			if( appRunning == -1 || response == 's' || response == 'S' ){
				...
				LLSEremoveOf(input, &installedApps);
				
				if( appRunning > -1 ){
					cout << "HEEEEREE!222222222";
					LLSEremoveOf(appRunning, &runningApps);
				}
...
void initRunning(){
	...
	int length = LLSEcountApps(runningApps);
	...
	if( input == 0 ){
		...
		input = LLSEgetTheIndex(input, runningApps);

		cout << runningApps.list[input].content.name << " fechado.\n";
		
		sleep(1000);
		LLSEremoveOf( input, &runningApps );
		initHome();
	}
}
